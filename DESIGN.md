...
> Status: **design draft** (no code yet). Standalone CLI in its own workspace.
...
- No write/delete calls in any provider module, ever. Remediation is the human's (or a
  future explicit `--fix` that only edits in-repo files, never touches provider state).
...
```
costguard audit [workspaces...] [--all] [--ci-only] [--crons-only] [--providers <list>]
costguard scan  [--ci] [--crons]                  # static only, alias subset
costguard providers --check                       # token presence/scope
costguard report --last [--json]                  # re-render last run
costguard registry --list | --validate            # inspect workspaces.json
```
...
- **Phase 1 — Half A.** Orchestrator + registry + CI rules + cron rules + reporter.
  Zero credentials. Proves value across every workspace immediately. **Ship first.**
- **Phase 2 — Supabase + GitHub billing modules.** The two real-leak providers.
- **Phase 3 — Railway / Netlify / Neon modules.** Round out coverage.
- **Phase 4 — `--fix` (in-repo only) + PR emission.** Deterministic, autofixable rules only.
- **Phase 5 — scheduled monthly digest** (`/schedule` or cron) posting the report.
